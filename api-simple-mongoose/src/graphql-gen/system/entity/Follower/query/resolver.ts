...
export const query: { [key: string]: any } = {
  followers: async (
    ...
    let list = get(selectionSet, 'edges.node') ? await context.connectors.Follower.getList({
      ...args,
      idMap,
    }) : [];
      ...
      let pageInfo = get(selectionSet, 'pageInfo') ?
        {
          ...
          count: get(selectionSet, 'pageInfo.count') ?  await fixCount(list.length, cursor, () => context.connectors.Follower.getCount({
              ...args,
              idMap,
            })) : 0,
        } : null;
