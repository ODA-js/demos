...
export const query: { [key: string]: any } = {
  files: async (
    ...
    let list = get(selectionSet, 'edges.node') ? await context.connectors.File.getList({
      ...args,
      idMap,
    }) : [];
      ...
      let pageInfo = get(selectionSet, 'pageInfo') ?
        {
          ...
          count: get(selectionSet, 'pageInfo.count') ?  await fixCount(list.length, cursor, () => context.connectors.File.getCount({
              ...args,
              idMap,
            })) : 0,
        } : null;
