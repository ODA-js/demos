...
export default class RegisterConnectors {
  ...
  public InitUser(): UserConnector {
    if (!this._User) {
      this._User = new User({ sequelize: this.sequelize, connectors: this, user: this.user, owner: this.owner, acls: this.acls, userGroup: this.userGroup });
    }
    return this._User;
  }
  ...
  public InitToDoItem(): ToDoItemConnector {
    if (!this._ToDoItem) {
      this._ToDoItem = new ToDoItem({ sequelize: this.sequelize, connectors: this, user: this.user, owner: this.owner, acls: this.acls, userGroup: this.userGroup });
    }
